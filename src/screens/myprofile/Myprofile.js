import {GridList,GridListTile,DialogContent,DialogContentText,Button,DialogActions,Avatar, Box, Container, Icon, makeStyles, Typography, Modal, Dialog, DialogTitle, TextField, IconButton, FormHelperText } from "@material-ui/core";
import {useState,useEffect} from "react";
import { useHistory, useLocation } from "react-router";
import Header from "../../common/header/Header";

import EditIcon from '@material-ui/icons/Edit';


const useStyles = makeStyles({
    "about-header":{
        display:"flex",
        justifyContent:"space-between",
        width:"80%"
    },
    avatar:{
        width:"150px",
        height:"150px"
    },
    "about-about":{
        width:"70%"
    },
    "username":{
        display:"block",
        fontWeight:600,
        marginBottom:"1rem",
        fontSize:"1.2rem"
    },
    "fullname":{
        display:"flex",
        alignItems:"center"
    },
    editIcon:{
        borderRadius:"50%",
        marginLeft:".5rem",
        backgroundColor:"rgb(220, 0, 78)",
        color:"white",
        '&:hover':{
            backgroundColor:"rgb(220, 0, 78)"
        }
    }
  });

  
function Myprofile(props){
    // constants
    const history = useHistory();
    const location = useLocation();
    const classes = useStyles();
    const profilePicURL = "data:image/png;base64,";
    const userData = {};
    
    // states
    const [username,setUserName] = useState("");
    const [totalPostsCount,setTotalPostsCount] = useState(0);
    const [fullName,setFullName] = useState("Sai Darshan");
    const [editFullName,setEditFullName] = useState(fullName);
    const [editError,setEditError] = useState("hide");
    const [activePost,setActivePost] = useState(null);
    const [thumbnails,setThumbnail] = useState([]);
    // modal status for open and clsoe
    const [fullNameEditModalStatus,setFullNameEditModalStatus] = useState(false);
    // side effects
    useEffect(()=>{
        const baseURL = "https://graph.instagram.com/";
        const getMediaIDsURL = `me/media?fields=id,caption&access_token=${sessionStorage.getItem("access-token")}`;
        const URL = `${baseURL}${getMediaIDsURL}`;
    
        fetch(URL).then(res=>res.json()).then(resJSON=>{
            const userPosts = resJSON.data;

            userPosts.forEach((post)=>{
                const getFullPostByIdURL = `${post.id}?fields=id,media_type,media_url,username,timestamp&access_token=${sessionStorage.getItem("access-token")}`;
                const URL = `${baseURL}${getFullPostByIdURL}`;
    
                fetch(URL).then(res=>res.json()).then(resJSON=>{
                    const fullPost = {...post,...resJSON};
                    
                    if(thumbnails.length === 0){
                        setThumbnail(prevArr=>{
                            return [...prevArr,{id:fullPost.id,image:fullPost.media_url}];
                        });
                    }

                    userData[fullPost.id] = fullPost;

                    setUserName(fullPost.username);
                    setTotalPostsCount(userPosts.length);

                    console.log(userData);
                })
    
            });
        })
    },[])

    
    
    // functions
    const handleClose = ()=>{
        setFullNameEditModalStatus(false);
    };

    const updateFullNameHandler = ()=>{
        if(editFullName === ""){
            setEditError("show");
            return;
        }
        setFullName(editFullName);
        setEditError("hide");
        handleClose();
    }

    // render
    return(
        <>
            <Header {...props} history={history} location={location} profilePicURL={profilePicURL}></Header>
            <Container maxWidth="md">
                <Box className={classes["about-header"]}>
                    <Avatar src={profilePicURL} className={classes.avatar}></Avatar>
                    <Box className={classes["about-about"]}>
                        <Typography variant="p" className={classes.username}>
                            {username}
                        </Typography>
                        <Box display="flex" justifyContent="space-between" marginBottom="1.5rem">
                            <Typography variant="p" className={classes["info"]}>
                                Posts: {totalPostsCount}
                            </Typography>
                            <Typography variant="p">
                                Followers: 252
                            </Typography>
                            <Typography variant="p">
                                Following: 600
                            </Typography>
                        </Box>
                        <Box className={classes.fullname}>
                            <Typography variant="body2" style={{fontSize:"1rem"}}>
                            {fullName}
                            <IconButton className={classes.editIcon} onClick={()=>setFullNameEditModalStatus(true)}>
                                <EditIcon></EditIcon>
                            </IconButton>
                                <Dialog open={fullNameEditModalStatus} onClose={handleClose}>
                                    <DialogTitle>{"EDIT"}</DialogTitle>
                                    <DialogContent>
                                        <TextField label="Full Name *" value={editFullName} onChange={(e)=>setEditFullName(e.currentTarget.value)} />
                                        <FormHelperText style={{color:"red"}} className={editError} >required</FormHelperText>
                                    </DialogContent>
                                    <DialogActions style={{display:"flex",justifyContent:"flex-start",padding:"1.5rem"}}>
                                    <Button variant="contained" onClick={updateFullNameHandler} color="primary">
                                        UPDATE
                                    </Button>
                                    </DialogActions>
                                </Dialog>
                            </Typography>
                        </Box>
                    </Box>
                </Box>
            </Container>

            <Container maxWidth="md" style={{marginTop:"3rem"}}>
                <GridList cellHeight={160} className={classes.gridList} cols={3}>
                    {thumbnails.map((thumbnail) => (
                    <GridListTile key={thumbnail.id} rows={2}>
                        <img src={thumbnail.image} data-id={thumbnail.id} style={{maxWidth:"450px",maxHeight:"350px",cursor:"pointer"}}/>
                    </GridListTile>
                    ))}
            </GridList>
            </Container>
        </>
    )
}

export default Myprofile;