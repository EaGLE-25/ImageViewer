...
function Myprofile(props){
    ...
    const [fullName,setFullName] = useState("Sai Darshan");
    const [editFullName,setEditFullName] = useState(fullName);
    const [editError,setEditError] = useState("hide");
    const [activePost,setActivePost] = useState(null);
    const [thumbnails,setThumbnail] = useState([]);
    // modal status for open and clsoe
    const [fullNameEditModalStatus,setFullNameEditModalStatus] = useState(false);
    // modal status for show image and close
    const [showImageModalStatus,setShowImageModalStatus] = useState(false);
    // side effects
    useEffect(()=>{
            ...
            userPosts.forEach((post)=>{
                ...
                fetch(URL).then(res=>res.json()).then(resJSON=>{
                    ...
                    if(thumbnails.length === 0){
                        setThumbnail(prevArr=>{
                            return [...prevArr,{id:fullPost.id,image:fullPost.media_url}];
                        });
                    }
                    ...
                    setUserData(prevObj=>{
                        const userDataObj = {...prevObj};
                        userDataObj[fullPost.id] = fullPost;

                        return userDataObj;
                    })
                    setUserName(fullPost.username);
    ...
    // functions
    const handleClose = ()=>{
        setFullNameEditModalStatus(false);
    };
    ...
    const imgClickHandler = (e)=>{
        setShowImageModalStatus(true);
        const clickedImageId = e.currentTarget.dataset.id;
        const clickedImage = userData[clickedImageId];

        console.log(clickedImageId,userData);
        
        setActivePost(clickedImage);
    }

    const handleImgClose = ()=>{
        setShowImageModalStatus(false);
    }
    ...
    return(
        <>
            ...
            <Container maxWidth="md">
                <Box className={classes["about-header"]}>
                    <Avatar src={profilePicURL} className={classes.avatar}></Avatar>
                    <Box className={classes["about-about"]}>
                        ...
                        <Box className={classes.fullname}>
                            ...
                            {fullName}
                            <IconButton className={classes.editIcon} onClick={()=>setFullNameEditModalStatus(true)}>
                                <EditIcon></EditIcon>
                            </IconButton>
                                <Dialog open={fullNameEditModalStatus} onClose={handleClose}>
                                    <DialogTitle>{"EDIT"}</DialogTitle>
                                    <DialogContent>
                                        <TextField label="Full Name *" value={editFullName} onChange={(e)=>setEditFullName(e.currentTarget.value)} />
                                        <FormHelperText style={{color:"red"}} className={editError} >required</FormHelperText>
                                    </DialogContent>
                                    <DialogActions style={{display:"flex",justifyContent:"flex-start",padding:"1.5rem"}}>
                                    <Button variant="contained" onClick={updateFullNameHandler} color="primary">
                                        UPDATE
                                    </Button>
                                    </DialogActions>
                                </Dialog>
            ...
            <Container maxWidth="md" style={{marginTop:"3rem"}}>
                <GridList cellHeight={160} className={classes.gridList} cols={3}>
                    ...
                    <GridListTile key={thumbnail.id} rows={2}>
                        <img alt="post media thumbnail" src={thumbnail.image} data-id={thumbnail.id} style={{maxWidth:"450px",maxHeight:"350px",cursor:"pointer"}} onClick={imgClickHandler} />
                    </GridListTile>
                    ))}
            </GridList>
            <Dialog open={showImageModalStatus} onClose={handleImgClose}>
                <Post {...activePost} profilePicURL = {profilePicURL} location={location}></Post>
            </Dialog>
            </Container>
        </>
    )
